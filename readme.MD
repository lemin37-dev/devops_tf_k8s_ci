...
- 전체 저장소 배치
  ...
  - 절차
    ...
    - 변경 : OIDC
        ...
        - 절차 : Git Action에서 ECR에 Push할 때 단기 AWS 자격인증 발급받아 로그인 수행
        - IAM 계정별 액세스키, PEM 파일 사용 X
...
- 수정 및 추가작업
    ...
    - git 관련 id 조회
        ```
        # github_owner_id, gitub_ci_repository_id 값 획득
        # github cli 필요
        # 설치
            # window
        ...
        gh auth login
        # ID 조회
        gh api repos/lemin37-dev/devops_tf_k8s_ci --jq "{owner_id: .owner.id, repository_id: .id, created_at: .created_at}"
        ```

## 전체 인프라 구성 및 최초 web/was 이미지 빌드 -> ECR push : ci workflow 진행
- 최초 (인프라 구성)
    ```
    .\project.sh|bat deploy
    ```

- ci setup 1회 진행
    ```
    .\setup-ci.sh|bat lemin37-dev/devops_tf_k8s_ci
    ```