...
docker run --detach --name "${WEB_CONTAINER}" --add-host was-service:127.0.0.1 --publish 80:80 "${WEB_IMAGE}" >/dev/null
...
docker run --detach --name "${WAS_CONTAINER}" --publish 8000:8000 "${WAS_IMAGE}" >/dev/null
wait_for_url "http://127.0.0.1:8000/health" "WAS"
